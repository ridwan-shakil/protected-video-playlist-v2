...
function pvp_enqueue_frontend_styles() {
    ...
    wp_enqueue_style(
        'pvp-grid-style',
        PVP_PLUGIN_URL . 'public/css/pvp-grid.css',
    ...
    $color   = isset( $options['theme_color'] ) ? pvp_sanitize_color( $options['theme_color'] ) : '#ff0000';

    $custom_css = ":root { --pvp-theme-color: {$color}; }";

    // Global branding colors
    $controls_bg_color = ! empty( $options['controls_bg_color'] ) ? pvp_sanitize_color( $options['controls_bg_color'] ) : '';
    $controls_color    = ! empty( $options['controls_color'] )    ? pvp_sanitize_color( $options['controls_color'] )    : '';
    $play_btn_bg_color = ! empty( $options['play_btn_bg_color'] ) ? pvp_sanitize_color( $options['play_btn_bg_color'] ) : '';
    $play_btn_color    = ! empty( $options['play_btn_color'] )    ? pvp_sanitize_color( $options['play_btn_color'] )    : '';
